...
def test_correlation_limit_applied_in_app(fake_download):
    ...
    at.sidebar.slider(key="pair_cap").set_value(30.0).run()
    assert not at.exception, at.exception
    opt, free = at.session_state["_opt_hist"], at.session_state["_opt_free"]
    assert opt["success"], opt["message"]
    tickers = list(opt["weights"])
    uni = D.load_universe(tickers, at.session_state["bench"], "5y", "BRL")
    corr = P.constraint_corr(M.log_returns(uni.prices[tickers]), "D", True)
    pairs = O.correlated_pairs(corr, 0.60)
    assert pairs
    w = [opt["weights"][t] for t in tickers]
    wf = [free["weights"][t] for t in tickers]
    assert all(w[i] + w[j] <= 0.30 + 1e-6 for i, j in pairs)
    assert free["success"] and np.isclose(sum(wf), 1.0)
